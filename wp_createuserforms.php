...
<form  method='post'  name='form' action="wp_formsinsert.php">
    <table border="1" align="left" >
    <tr><td>
    <p class="contact"><label for="name">New Form Title:</label></p></td><td>
    <input type='text' name='name' id='name' required=" " maxlength="50"  /></td></tr>
	 ...
	 <?php   $vvvv=mysql_query("SELECT distinct wp_properties.ppro_id,wp_properties.Name,wp_manage_mappings.ppro_id FROM wp_properties left join wp_manage_mappings on wp_properties.ppro_id=wp_manage_mappings.ppro_id where wp_manage_mappings.user_id='$uid'");
	   
	   while($row=mysql_fetch_array($vvvv))
	   {?>
	 <tr> <td colspan="2"> <input type="checkbox" name="ppro_id[]" value="<?php echo $row['ppro_id'];?>"><?php echo $row['Name'];?></td></tr>
