...
?>
<form action="updatecontrolpanel.php" method="post">
<input type="hidden" name="rid" value="<?php echo $id1;?>">
<input type="hidden" name="pid" value="<?php echo $id;?>">
<div><center>Control Panel</center></div><br><br>
<?php $query="select name,defaultallocation,defaultrate,defaultmin_stay,defaultinclusion from wp_room_type_details where 
ppro_id=$id and roomid=$id1";
$q=mysql_query($query);
$row1=mysql_fetch_array($q);?>
<div>
Room Type:<?php echo $row1['name'];?>
&nbsp;&nbsp;Date:<?php echo $newdate;?>
</div><hr>

<?php $qry="select * from wp_inventory_grid_details where 
ppro_id=$id and roomid=$id1 and grid_date='$newdate'";
$q1=mysql_query($qry);
...
<td>$<?php echo $row['roomrate'];?> 
<input type="hidden" name="roomrate" value="<?php echo $row['roomrate'];?>"></td>
</tr>
<tr>
<td>Stop Sell</td>
<td><input type="checkbox" name="check" value="check"></td>
<td><?php if($row['stop_sell']){;?>Yes<?php }else{?>No</td><?php }?></td>
</tr>
<tr>
<td>Min Stay</td>
<td><input type="checkbox" name="check" value="check"></td>
<td><?php echo $row['min_room_stay'];?></td>
</tr>
